...
(function () {
    ...
    EditStoryController.$inject = ['$scope', '$state', '$mdDialog', '$filter', '$timeout', 'UtilityFactory', 'SearchFactory', 'EditService', 'Gparams'];
    function EditStoryController($scope, $state, $mdDialog, $filter, $timeout, UtilityFactory, SearchFactory, EditService, Gparams) {
        ...
        vm.doChange4 = doChange4;				//prototype function declaration so can be called before defined
...
// ToDo: make it real for production
        ...
        function onSubmit4() {
            ...
            EditService.editStory(vm.data)
                .then(function successCallback(response) {
                    if (response.status === 200) {
                        UtilityFactory.showTemplateToast('<md-toast>The story was successfully updated in ' +
                            vm.params.appName + '.</md-toast>');
                        vm.onReset();

                    } else {
                        UtilityFactory.showAlertToast('<md-toast>' + response.error + '. Please reenter.</md-toast>');
                    }
                });
        }
        ...
        vm.trixFileAccept = function (e, editor) {
            ...
            if (filename.length > 2) {							// possible malware since more than one extension
                UtilityFactory.showAlertToast('<md-toast>Multiple file extensions are not allowed in ' +
                    vm.params.appName + ' since this is a strong indicator the file may contain malware.</md-toast>');
        ...
        $scope.showConfirm = function () {
            var confirm = $mdDialog.confirm()
                ...
                .textContent('The story will be permanently deleted and this can not be undone.')
                .ariaLabel('Delete Story')
                .ok('OK')
                .cancel('Cancel');

            $mdDialog.show(confirm).then(function () {
                UtilityFactory.showTemplateToast('<md-toast>The story with Title "' + vm.data.details.theTitle +
                    '" has been deleted.</md-toast>');
            }, function () {

/* ******************************************************
 ...
 ToDo: make spouses array real for current person
 ToDo: in php or jquery, prevent duplicate events where they don't make sense
...
(function () {    'use strict';
    ...
    EditMarriageController.$inject = [ '$scope', '$rootScope', '$state', '$mdDialog', '$filter', 'UtilityFactory', 'EditService', 'KfamilyEvStruct','KdateModifiers', 'Gparams'];
   		function EditMarriageController($scope,   $rootScope,   $state,   $mdDialog,   $filter,   UtilityFactory,   EditService,   KfamilyEvStruct,  KdateModifiers,   Gparams) {
            ...
            vm.doChange3 = doChange3;           	//prototype function declaration so can be called before defined
...
// ToDO: need to deal with multiple events of same type
// ToDo: what about multiple mothers/fathers?
...
*/
			...
			function doAdd3() {
   			...
   			function searchPage3(who,what) {
	        	$state.go('search', {obj:{name:'editMarriages', search:vm.data.surname, tab:vm.tab, what:what, who:who}});
	      	}


	      	function onSubmit3() {
	      		vm.data.userID = vm.params.curUserID;
				...
				EditService.updateAttributes(vm.data)
	           	.then(function successCallback(response) {
	  				if (response.status===200) {
		  				UtilityFactory.showTemplateToast('<md-toast>The ' + vm.data.selectedType.label +
                            ' event for ' + vm.data.fullName + ' has been added to ' + vm.params.appName + '.</md-toast>');
	  					vm.onReset();

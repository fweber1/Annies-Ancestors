...
// initialization code 
			...
			vm.tab = 1;			
  			...
  			vm.doChange0 = doChange0;														//prototype function declaration so can be callled before defined
   			vm.init0 = init0;
   			vm.onReset = onReset;
   			vm.doAdd0 = doAdd0;
  					...
  					vm.data = UtilityFactory.setFamily(vm.params.curPersonID)
  		           	.then(function successCallback(response) {
  		  				if (response.status=='200') {
  							...
  							UtilityFactory.showAlertToast('<md-toast> An error occurred during the update of ' + vm.fullName+ ' the error was: ' + vm.response.theError + '. ' + vm.params.appName + ' has been notified.</md-toast>');
			...
			init0();
   			...
   			function init0() {		
				...
				nameDropzone = new Dropzone("div#uploadProfile0", {
					...
					renameFilename			: function (name) {return vm.data.mainID + '.' + name.substr(name.lastIndexOf('.')+1);},
...
//functions
			...
			function onSubmit0() {
				EditService.UpdateName(vm.data)
	           	.then(function successCallback(response) {
	  				if (response.status=='200') {
						UtilityFactory.showTemplateToast('<md-toast>' + vm.fullName+ ' was successfully updated in ' + vm.params.appName + '.</md-toast>');

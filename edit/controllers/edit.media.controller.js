...
(function () {    'use strict';
    ...
    EditMediaController.$inject = [ '$scope', '$state', '$mdDialog', 'EditService', '$filter', 'Gparams', 'UtilityFactory'];
   		function EditMediaController($scope,   $state,   $mdDialog,   EditService,   $filter,   Gparams,   UtilityFactory) {
			...
			vm.doChange5 = doChange5;														//prototype function declaration so can be callled before defined
...
*/
			...
			$scope.showConfirm = function() {
			    var confirm = $mdDialog.confirm()
			          ...
			          .textContent('The media will be permamently deleted and this can not be undone.')
			          .ariaLabel('Delete Item')
			          .ok('OK')
			          .cancel('Cancel');

			    $mdDialog.show(confirm).then(function() {
	  				UtilityFactory.showTemplateToast('<md-toast>The media named ' + vm.data.details.theTitle + ' about ' + vm.data.fullName + ' has been deleted.</md-toast>');
			    }, function() {

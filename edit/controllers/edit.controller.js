...
'use strict';
  	...
  	angular.module('root').controller('EditController', EditController);
    	...
    	EditController.$inject = ['$scope', '$rootScope', '$state', '$stateParams', 'UtilityFactory', 'Gparams'];
   		function EditController(   $scope,   $rootScope,   $state,   $stateParams,   UtilityFactory,   Gparams) {
			...
			if(vm.params.debug) {
				...
				$scope.email.editors = [{mainID:6121, fullName:'Hazel Johnson'}, {mainID:6122, fullName:'Sarah Johnson'}, {mainID:6123, fullName:'Angela Johnson'}];																// NOTE: need to make this live
			} else {

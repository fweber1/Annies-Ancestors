...
$which = "main.mainID, events.ownerID, spouseID, theType, dateModifier, date1, date2, phrase, city, state, county, country, details, ";
$which = $which . "surname, CONCAT_WS( ' ', givenName, surname) AS fullName";
$where = "main.mainID = $personID AND mainType = 'FAMI'";
...
// get spouse IDs and then spouse names

$which = "spouseID, startDate AS date1, endDate AS date2";
...
for ($i=0;$i<mysqli_num_rows($sql);$i++) {
	...
	$which = "mainID, surname, CONCAT_WS( ' ', namePrefix, givenName,  middleName, surname, nameSuffix) AS fullName";
	$where = "mainID = $id[spouseID]";
	$sql1 = mysqli_query($link, "SELECT $which FROM main WHERE $where");
	$tmp = mysqli_fetch_assoc($sql1);
	$result[$i+$theLen] = array_merge(array('what' => 'spouse'), array('date1' => $id['date1']), array('date2' => $id['date2']), $tmp);
}

...
$data = "ownerID=6120&mainID=6121&fatherID=6120&motherID=6126&theType=BIR&dateModifier=EXA&date1=11/07/1952&date2=11/07/1995&phrase=&city=Minneapolis&state=Minnesota&county=Hennipen&country=USA&details=Hackley Hospital";
...
if(isset($output['fatherID'])) {$fatherID = $output['fatherID'];} else {$fatherID = '';};
if(isset($output['motherID'])) {$motherID = $output['motherID'];} else {$motherID = '';};
...
$fatherID = test_input($link, $fatherID);
$motherID = test_input($link, $motherID);
...
$sql = mysqli_query($link, "INSERT INTO events ($which) VALUES ($value)");
...
if($fatherID=='Unknown') $fatherID = -1;
if($motherID=='Unknown') $motherID = -1;

if($fatherID!=-1 || $motherID!=-1) {										// have mother and/or father, so add to family DB
	$which = "ownerID,mainID,spouseID,childID,startDate,endDate,CREATEDATE";
	$value = "'$ownerID','$fatherID','$motherID','$mainID','$date1','$date2','$createDate'";
	...
	$which = "ownerID,mainID,spouseID,childID,startDate,endDate,CREATEDATE";
	$value = "'$ownerID','$motherID','$fatherID','$mainID','$date1','$date2','$createDate'";

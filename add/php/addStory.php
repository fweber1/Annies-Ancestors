...
if (!preg_match("/^[a-zA-Z ]*$/",$theType)) {
	$error = "Only letters and white spaces are allowed in the theType";
	returnWithError($error, $link);
}

if (!preg_match("/^[a-zA-Z ]*$/",$theFile)) {
	$error = "Only letters and white spaces are allowed in the the file name";
	returnWithError($error, $link);
}
